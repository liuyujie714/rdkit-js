...
RUN emcmake cmake -DRDK_BUILD_FREETYPE_SUPPORT=ON -DRDK_BUILD_MINIMAL_LIB=ON \
  ...
  -DRDK_BUILD_MINIMAL_LIB_RGROUPDECOMP=ON -DRDK_BUILD_AVALON_SUPPORT=ON \
  ...
  -DCMAKE_CXX_FLAGS="-I/opt/expat/include -I/src/rdkit ${EXCEPTION_HANDLING} -O3 -DNDEBUG" \
  -DCMAKE_C_FLAGS="${EXCEPTION_HANDLING} -O3 -DNDEBUG -DCOMPILE_ANSI_ONLY" \
  -DCMAKE_EXE_LINKER_FLAGS="${EXCEPTION_HANDLING} -s STACK_OVERFLOW_CHECK=1 -s USE_PTHREADS=0 -s ALLOW_MEMORY_GROWTH=1 -s MAXIMUM_MEMORY=4GB -s MODULARIZE=1 -s STACK_SIZE=5MB -s EXPORT_NAME=\"'initRDKitModule'\"" ..
